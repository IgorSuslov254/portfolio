<?php
	...
	*/
	...
	$first_date = strtotime('0:00 first day of previous month', time());
    $last_date  = strtotime('23:59 last day of previous month', time());
    $sar_date = strtotime($SAR_period_start);
	if( ( date('Y', $sar_date) == date('Y') && date('m', $sar_date) == date('m') ) || 
	    ( date('d') < 8 && $sar_date >= $first_date && $sar_date <= $last_date ) ) {
		$change_data = true;
	} else {
		$change_data = false;
	}
	$change_data_DSR = date('Y', $sar_date) == date('Y') && date('m', $sar_date) == date('m');
	//echo '<h1 style="float:right;">'.(int)$change_data.'</h1>';
	...
	include "sar_view_css_v4.css";
	
?>

<!-- <div id="version" style="display:block;">SAR_V4</div> -->
<!-- loading -->
		...
		<section class="content">
			<form method="post" action="<?=base_url(SAR_URL)?>" >
				...
				<div class="inputs">
					...
					<a class="btn btn-primary" href="<?=base_url(SAR_URL.'/excel_dsr_report').'?SAR_period_start='.$sar_total_date['SAR_period_start']?>" style="margin-right:27px;" data-toggle="tooltip" title="Daily Sales Report експорт в Excel" target="_blank">DSR Report</a>
...
<!-- DSR -->
			...
			<div class="DSR_block">
				<input type="hidden" name="DSR_count" value="<?=count($DSR_data)?>">
				<?php 
					$dsr_name = "Sale plan and forecast (план і прогноз продажів на місяць)"; 
					$dsr_name = "Daily Sales Report";
					//$DSR_titles = ['', 'План продажу на місяць', 'Прогноз продажу на місяць'];
					
					//$DSR_data = $SAR_index['0']['value'];
					$index = $SAR_index['0'];
				?>
				
				<?php if( count($SAR_selected_dealer) > 1 || $SAR_selected_dealer[0] == 'XXX' || !empty($SAR_dealers) || $change_data_DSR == false): ?>
					<p class="title_sar"><?=$DSR_index['name']?></p>
				<?php else: ?>
					<p class="title_sar" style="background: #26761f;"><?=$DSR_index['name']?></p>
				<?php endif; ?>	
				
				<table class="sar_traffic">
					
					<?php foreach ($DSR_index['titles'] as $row):?>
						<tr>
							<?php foreach ($row as $col):?>
								<?php if (isset($col['title'])): ?>
									<th <?=$col['rowspan']?'rowspan="'.$col['rowspan'].'"':($col['colspan']?'colspan="'.$col['colspan'].'"':'')?>><?=$col['title']?></th>
								<?php endif; ?>
							<?php endforeach; ?>	
						</tr>
					<?php endforeach; ?>
					
					<?php foreach ($DSR_data as $row_index => $data_item):?>
					
						<tr <?=$data_item['isTotal']==true?'class="total_row"':'';?>>
							<?php foreach ($DSR_index['cols'] as $col_index => $col ):?>
							
								<?php 
									$cc_title = '';
									if ( isset($data_item[$col.'_tooltip']) &&
									    (count($SAR_selected_dealer) > 1 || $SAR_selected_dealer[0] == 'XXX' || !empty($SAR_dealers))
									) {
										foreach($data_item[$col.'_tooltip'] as $t_name => $t_value) {
											$cc_title .= $t_value. ' '.$t_name.'<br>';
										}
									} 
								?>
							
								<?php if ( !in_array($col, array_keys($DSR_index['updatable'])) || $data_item['isTotal'] ==true || 
								               count($SAR_selected_dealer) > 1 || $SAR_selected_dealer[0] == 'XXX' || !empty($SAR_dealers)  
											   || $change_data_DSR == false):?>
									<?php if ($col_index==0): ?>
										<?php $style = 'width: 150px;text-align: left;'; ?>
										<?php if ($data_item['fromCarsList'] === false):?>
											<?php $style .= 'background: #f88;'; ?>
										<?php elseif ($data_item['isTotal']==true): ?>
											<?php $style .= 'background: #0070a1;color:white;font-weight:500;'; ?>
										<?php endif; ?>
										<input type="hidden" name="DSR_car_id_<?=$row_index?>" value="<?=$data_item['car_id']?>">
									<?php elseif ($data_item['isTotal']==true): ?>
										<?php $style = 'background: #0070a1;color:white;font-weight:500;'; ?>
									<?php else: ?>
										<?php $style = 'background: #ecf0f5;'; ?>
									<?php endif; ?>
									<td data-html="true" data-toggle="tooltip" title="" data-original-title="<?=$cc_title?>" style="<?=$style?>"><?=$data_item[$col]?></td>
								<?php else: ?>
									<input type="hidden" name="DSR_old_value_<?=$col.'_'.$row_index;?>" value="<?=$data_item[$col]?>">
									<?php
										/*$index['value'] == 'SAR_leads' ? $sar_min = $showroom_traffic[$i]['value'] : $sar_min = 0;*/
										$sar_min = 0;
									?>

									<td><input title= "<?=$cc_title?>" type="number" min="0" name="DSR_<?=$col.'_'.$row_index;?>" value="<?=$data_item[$col]?>"></td>
								<?php endif; ?>
							<?php endforeach; ?>
						</tr>
					<?php endforeach; ?>		
					...
					<tr class="total_row">
						<?php foreach ($DSR_index['cols'] as $col):?>
							<td><?=$DSR_total[$col]?></td>
						<?php endforeach; ?>
					</tr>
...
<!-- End of DSR -->
				...
				<?php foreach ($SAR_index as $index):?>
					<?php if (!empty(${$index['value']})): ?>
						...
						<?php if(($index['updatable'] === false) || 				
						 (is_array($SAR_selected_dealer) && (count($SAR_selected_dealer) > 1 || $SAR_selected_dealer[0] == 'XXX')) || 
						!empty($SAR_dealers) || $change_data === false ): ?>
						...
						<div style="overflow-x: auto;">
							<table class="sar_traffic <?=$index['value']?>">
								...
								<?php $j=0; foreach (${$index['value']} as $showroom_traffic):?>
									<tr>
										<?php for ($i=0; $i<=32; $i++):?>
											<?php if (($i==0 || $i==32) || ($index['updatable'] === false) || 
											( is_array($SAR_selected_dealer) && (count($SAR_selected_dealer) > 1 || $SAR_selected_dealer[0] == 'XXX')) || 
											!empty($SAR_dealers)|| $change_data === false  || $i > $array_date[2] ):?>
												<?php if ($i==0): ?>
													<?php $style = 'width: 150px;text-align: left;'; ?>
